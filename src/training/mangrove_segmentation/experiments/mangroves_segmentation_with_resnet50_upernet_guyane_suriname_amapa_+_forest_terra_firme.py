...
OUT_DIR = BASE2 / "Mangrove_CROMA_Project/data/segmentation_binaire_croma_resnet_upernet_guyane_suriname_amapa_v2"
...
class ResNet50BigEarthNetBackbone(nn.Module):
    def __init__(self, weight_path=None, in_channels=12):
        ...
        self.body = IntermediateLayerGetter(resnet, return_layers={"layer1": "c1", "layer2": "c2", "layer3": "c3", "layer4": "c4",})
...
class PPM(nn.Module):
    def __init__(self, in_channels=2048, out_channels=512, pool_scales=(1,2,3,6)):
        super().__init__()
...
class FPN(nn.Module):
    ...
    def __init__(self, channels=[256,512,1024,512], out_channels=256):
        super().__init__()
        self.lateral = nn.ModuleList()
        self.output = nn.ModuleList()

        for c in channels:
            self.lateral.append(nn.Conv2d(c,out_channels,1))
            self.output.append(
                nn.Sequential(
    ...
    def forward(self,features):
        ...
        p2 = self.lateral[1](c2)
        p1 = self.lateral[0](c1)
...
class UPerHead(nn.Module):
    ...
    def __init__(self,out_channels=256):
        ...
        self.cls = nn.Conv2d(out_channels,1,1)
...
class UPerNet_S1S2(nn.Module):

    def __init__(self, weight_path=None):
        super().__init__()
        self.backbone = ResNet50BigEarthNetBackbone(weight_path=weight_path, in_channels=12)
        self.ppm = PPM(in_channels=2048, out_channels=512)
        self.fpn = FPN(channels=[256,512,1024,512], out_channels=256)
        self.head = UPerHead(out_channels=256)
    ...
    def forward(self,x):
        ...
        c2 = feats["c2"]
        c3 = feats["c3"]
        c4 = self.ppm(feats["c4"])
        p1,p2,p3,p4 = self.fpn([c1,c2,c3,c4])
        out = self.head(p1,p2,p3,p4)
        out = F.interpolate(out, size=x.shape[-2:], mode="bilinear", align_corners=False)
        return out

# INITIALISATION
model = UPerNet_S1S2(weight_path=str(WEIGHTS)).to(DEVICE)
...
class FocalDiceLoss(nn.Module):
    ...
    def __init__(self, alpha=0.75, gamma=2, pos_weight=4.0, smooth=1e-6):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
        self.smooth = smooth
...
criterion = FocalDiceLoss(alpha=0.75,gamma=2, pos_weight=pos_weight)

# OPTIMIZER
optimizer = torch.optim.AdamW([
        {"params": model.backbone.parameters(), "lr": 5e-5},
        {"params": model.ppm.parameters(), "lr": 5e-5},
        {"params": model.fpn.parameters(), "lr": 5e-5},
...
for epoch in range(start_epoch, EPOCHS):
    ...
    if val_iou > best_iou + 1e-4:
        ...
        torch.save(model.state_dict(), OUT_DIR / "best_resnet50_and_upernet.pth")
        torch.save({
        "epoch": epoch,
...
experiment_name = "resnet50_and_upernet_BigEarthNet_S1S2"
...
# TEST EVALUATION
logging.info("▶ TESTING S1 + S2 with ResNet50 pretrained with BigEarthNet + UPerNet")

model =  UPerNet_S1S2(weight_path=str(WEIGHTS)).to(DEVICE)
state_dict = torch.load(OUT_DIR / "best_resnet50_and_upernet.pth", map_location=DEVICE)
...
for threshold in thresholds:
    ...
    logging.info("▶ Reconstructing rasters...")
    prediction_files = []
    for tif in sorted(test_df["tif"].unique()):
        logging.info(f"Processing {Path(tif).stem}")
        rows = test_df[test_df["tif"] == tif]
        try:

            with rasterio.open(tif) as src:
                profile = src.profile.copy()
                pred_sum = np.zeros((src.height, src.width), dtype=np.float32,)
                pred_count = np.zeros((src.height, src.width), dtype=np.uint16,)

        except Exception as e:

            logging.warning(f"Cannot open {tif}: {e}")
            continue
        ...
        for _, row in rows.iterrows():
            ...
            try:
                with rasterio.open(row["tif"]) as s1:
                    p1 = s1.read(window=Window(x, y, PATCH_SIZE, PATCH_SIZE,))

                with rasterio.open(row["s2"]) as s2:
                    p2 = s2.read(window=Window(x, y, PATCH_SIZE, PATCH_SIZE,))

                patch = np.concatenate([p1, p2], axis=0)
                patch = torch.tensor(patch, dtype=torch.float32,).unsqueeze(0)
                patch /= patch.max() + 1e-6
        ...
        pred_mean = pred_sum / np.maximum(pred_count, 1)
        pred_bin = (pred_mean > threshold).astype(np.uint8)

        # Save raster immediately
        out_file = (RASTER_DIR / f"{Path(tif).stem}_prediction.tif")
    ...
    logging.info("✔ Raster reconstruction finished")

    # VECTORIZATION WITHOUT MOSAIC
    logging.info("▶ Vectorizing prediction rasters...")
    all_geometries = []
    crs = None
    for raster_file in prediction_files:
        logging.info(f"Vectorizing {raster_file.name}")

        with rasterio.open(raster_file) as src:
            raster = src.read(1)
            crs = src.crs
            transform = src.transform
            for geom, value in shapes(raster, mask=raster == 1, transform=transform,):
                if value == 1:
    ...
    # SAVE SHAPEFILE
    gdf = gpd.GeoDataFrame(geometry=all_geometries, crs=crs,)
    shp_file = (exp_dir / "predicted_mangroves_with_s1_s2.shp")
    gdf.to_file(shp_file)
    logging.info(f"✔ {len(gdf)} polygons written to {shp_file}")
