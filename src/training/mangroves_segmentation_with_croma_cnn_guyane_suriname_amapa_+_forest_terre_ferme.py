...
OUT_DIR = BASE2 / "Mangrove_CROMA_Project/data/segmentation_binaire_croma_cnn_guyane_suriname_amapa_v2"
...
class LightweightDecoder(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Upsample(size=(PATCH_SIZE, PATCH_SIZE), mode="bilinear", align_corners=False),
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        enc = self.encoder(SAR_images=s1, optical_images=s2)

        # EXTRAIRE les bonnes features
        enc = enc["joint_encodings"]

        B, N, C = enc.shape
        s = int(np.sqrt(N))
        enc = enc.reshape(B, s, s, C).permute(0, 3, 1, 2)
...
class DiceBCELoss(nn.Module):
    def __init__(self, pos_weight=16.0):
        super().__init__()
...
optimizer = torch.optim.AdamW(model.decoder.parameters(), lr=LR, weight_decay=5e-4)
...
for epoch in range(start_epoch, EPOCHS):
    ...
    if val_iou > best_iou + 1e-4:
        ...
        torch.save(model.state_dict(), OUT_DIR / "best_croma_cnn.pth")
        torch.save({
        "epoch": epoch,
...
BASE_OUT = BASE / "outputs_s1_s2"
BASE_OUT.mkdir(parents=True, exist_ok=True)

experiment_name = "LightCNN_CROMA_S1S2"
...
state_dict = torch.load(OUT_DIR / "best_croma_cnn.pth", map_location=DEVICE)
