...
OUT_DIR = BASE / "Mangrove_CROMA_Project/scripts/training/segmentation_habitats/segmentation_dataset_croma_upernet_multiclass__partial_finetuning"
...
class MangroveDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # on ignore entièrement un patch corrompu
...
class CROMA_UPerNet(nn.Module):
    ...
    def __init__(self, num_classes, n_unfrozen_blocks=2,):
        super().__init__()
        ...
        # Sentinel-1 encoder
        for block in self.encoder.s1_encoder.transformer.layers[-n_unfrozen_blocks:]:
            for p in block.parameters():
                p.requires_grad = True

        # Sentinel-2 encoder
        for block in self.encoder.s2_encoder.transformer.layers[-n_unfrozen_blocks:]:
            for p in block.parameters():
                p.requires_grad = True

        # Joint SAR-optical encoder
        for block in self.encoder.cross_encoder.layers[-n_unfrozen_blocks:]:
            for p in block.parameters():
                p.requires_grad = True

        # Also unfreeze final LayerNorms
        for p in self.encoder.s1_encoder.transformer.norm_out.parameters():
            p.requires_grad = True

        for p in self.encoder.s2_encoder.transformer.norm_out.parameters():
            p.requires_grad = True

        for p in self.encoder.cross_encoder.norm_out.parameters():
            p.requires_grad = True
...
NUM_CLASSES = 4
model = CROMA_UPerNet(num_classes=NUM_CLASSES, n_unfrozen_blocks=2,).to(DEVICE)
...
class OrdinalFocalLoss(nn.Module):
    ...
    def __init__(self, num_classes=NUM_CLASSES, ignore_index=255, ordinal_weight=0.2, gamma=2.0, class_weights=None,):
        super().__init__()
        ...
        self.focal = FocalLoss(gamma=gamma, ignore_index=ignore_index, weight=class_weights,)
...
criterion = OrdinalFocalLoss(num_classes=4, ignore_index=255, ordinal_weight=0.2, gamma=2.0,)        
...
optimizer = AdamW([
        ...
        {"params": model.proj.parameters(), "lr": LR_DECODER,},
        {"params": model.ppm.parameters(), "lr": LR_DECODER,},
        {"params": model.head.parameters(), "lr": LR_DECODER,},], weight_decay=1e-4,)
...
print(f"Trainable proportion : " f"{100 * trainable / (trainable + frozen):.2f}%")

# TRAINING
CHECKPOINT = OUT_DIR / "checkpoint_croma_upernet_partial_finetuning.pt"
BEST_MODEL = OUT_DIR / "best__croma_upernet_partial_finetuning.pth"
...
with torch.no_grad():
    for patches, masks, _, _, _ in test_loader:
        ...
        indices = (true_valid * NUM_CLASSES + pred_valid)
