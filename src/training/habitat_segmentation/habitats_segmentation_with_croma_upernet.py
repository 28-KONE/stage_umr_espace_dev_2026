...
OUT_DIR = BASE / "Mangrove_CROMA_Project/scripts/training/segmentation_habitats/segmentation_dataset_croma_upernet_multiclass"
                        ...
                        for item in candidate_s2:
                            ...
                            for y in range(0, src1.height - PATCH_SIZE + 1,  STRIDE):
                                for x in range(0,  src1.width  - PATCH_SIZE + 1,  STRIDE):
                                    ...
                                    # KEEP ONLY PATCHES CONTAINING AT LEAST ONE LABELED MANGROVE
                                    # HABITAT PIXEL
...
optimizer = AdamW([{"params": model.proj.parameters(), "lr": 5e-4}, {"params": model.ppm.parameters(), "lr": 5e-4}, {"params": model.head.parameters(), "lr": 5e-4},], weight_decay=1e-4)


# TRAINING
CHECKPOINT = OUT_DIR / "checkpoint.pt"
BEST_MODEL = OUT_DIR / "best_croma_upernet_multiclass.pth"
...
for epoch in range(start_epoch,EPOCHS):
    ...
    if val_iou > best_iou + 1e-4:
        ...
        torch.save(model.state_dict(), OUT_DIR / "best_croma_upernet_multiclass.pth")
